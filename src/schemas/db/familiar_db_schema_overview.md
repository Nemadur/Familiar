...
> This document describes what’s already implemented in the database schema **v0.2.9** and how the data is structured.
...
- **Privacy**: profiles and characters can be public or private; private content is visible to accepted followers.
...
- **Characters (Character/OC)**
  - `characters` + `character_reference_sheets`
  - `post_character_refs` links posts to characters (Character gallery)
- **Commissions**
...
- `report_target_type`: `user | post | shop_item | commission_listing | character | message`
...
- Used by:
  ...
  - `character_reference_sheets.asset_id`
...
### Character gallery
- `post_character_refs(post_id, character_id)`
- Gallery is generated by joining `post_character_refs → posts → post_media`.
...
- Pricing:
  ...
  - `discount_rate` (0..1)
- “Premade sections”:
  - `service_type` (`custom_service` / `percharacterlized_ych`)
...
- **characters**: viewable if owner OR (owner is viewable AND character is not private) OR accepted follower
...
## Character page
- `characters` + `character_reference_sheets → media_assets`
- `post_character_refs → posts → post_media → media_assets` for gallery
...
- If you want “public character but private owner”, you can decouple character visibility from owner visibility (currently it follows owner privacy + own flag).
