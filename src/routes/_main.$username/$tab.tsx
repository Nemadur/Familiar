...
function UserFeedContent({ user, tab }: { user: TUserProfile, tab: string }) {
	const content = useSuspenseProfileContent(user.userId, tab as any);

	switch (tab) {
		case "commissions":
			return <DefaultProfileTabContent username={user.username} />;
		case "portfolio":
			return <ProfilePortfolio posts={content?.posts || []} folders={content?.folders || []} />;
		case "characters":
			return <ProfileCharacters characters={content?.characters || []} />;
		case "shop":
			return <EmptyPage icon={OutlineReceipt} title="Shop" description="Shop feature is coming soon!" />;
		case "liked":
		case "saved":
			return <ProfileFeed posts={content?.posts || []} variant="feed" />;
		default:
			return <div className="p-4 text-center text-muted-foreground">Tab not found</div>;
	}
}
