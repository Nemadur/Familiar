...
function RouteComponent() {
	const { username, tab, folderSlug, subfolderSlug } = Route.useParams();
	...
	if (tab === "portfolio") {
		...
		const parentFolder = portfolioFolders.find(
			(f) => f.slug === folderSlug || f.id === folderSlug,
		);

		const currentFolder = portfolioFolders.find(
			(f) =>
				(f.slug === subfolderSlug || f.id === subfolderSlug) &&
				(parentFolder ? f.parentId === parentFolder.id : true),
		);
	...
	return (
		<div>
			Detail for {tab} - {folderSlug}/{subfolderSlug}
		</div>
	);
}
