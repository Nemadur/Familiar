...
type PortfolioPostWithComments =
    PortfolioPostResponse & {
        commentCount?: number;
        commentsCount?: number;
    };

export const PortfolioFeedItem = memo(
    function PortfolioFeedItem({
        p,
        post,
        handlePostClick,
        style,
        ...
        const hasContentWarnings =
            (post.contentWarnings?.length ?? 0) > 0;

        const postWithComments =
            post as PortfolioPostWithComments;

        const likeCount = post.likeCount ?? 0;

        const commentCount =
            postWithComments.commentCount ??
            postWithComments.commentsCount ??
            0;
