...
export const PortfolioFeedItem = memo(function PortfolioFeedItem({
	...
	const medias = useMemo<NormalizedMedia[]>(
		() =>
			[...(post.images ?? [])]
				...
				.map((image, index) => {
					const path = image.fullSize?.path ?? image.thumbnail?.path ?? "";
					...
					return {
						...
						alt:
							post.title ??
							t("components.portfolio.post.image_alt", "Portfolio image"),
					};
				})
	...
	const formatCount = (count: number) => count.toLocaleString(i18n.language);

	return (
		<article
			className="portfolio-feed-item group relative cursor-pointer isolate overflow-hidden rounded-p"
			style={{
				...style,
				minWidth: 160,
				minHeight: 160,
		...
		>
					...
					{firstMedia ? (
						...
						<div className="flex size-full flex-col items-center justify-center gap-2 bg-muted/30 p-4 text-center">
							...
							<p className="line-clamp-2 max-w-36 text-xs font-medium leading-snug text-muted-foreground">
								{post.title ||
									t("components.portfolio.post.no_media", "No media available")}
							</p>
						</div>
					)}
				</ContentWarningOverlay>
			...
			{hasMedia && hasMultipleImages && !hasContentWarnings && (
				<div className="pointer-events-none absolute right-2 top-2 z-20 flex size-7 items-center justify-center rounded-full bg-black/50 text-white backdrop-blur-sm">
					<Images aria-hidden="true" className="size-3.5" />
				</div>
			)}

			{/* Instagram-like engagement overlay */}
			...
			{/* Normal posts open through this target */}
			{!hasContentWarnings && (
				<button
					type="button"
					className={cn(
						"absolute inset-0 z-10 cursor-pointer rounded-p",
						"focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-ring",
					)}
					aria-label={
						post.title
							? `View post: ${post.title}`
							: t("components.portfolio.post.view", "View portfolio post")
					}
			...
			<div
				className={cn(
					"absolute left-2 top-2 z-40 transition-opacity duration-150",
					isMenuOpen ? "opacity-100" : "opacity-0 group-hover:opacity-100",
				)}
			>
				<DropdownMenu
				...
				>
					<DropdownMenuTrigger asChild>
						<Button
							...
							className="size-8 rounded-full border-0 bg-black/45 text-white backdrop-blur-sm hover:bg-black/65 hover:text-white"
					...
					>
						...
						>
							...
							{t("components.profile.portfolio.manage.delete", "Delete post")}
						</DropdownMenuItem>
