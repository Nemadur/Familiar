...
import { useLifelineAxisLabels } from "./lifeline-labels";
...
function LifelineVerticalEvent({ event }: { event: LifelineEvent }) {
	...
	const isInteractive = Boolean(image || (effect && fireworks));

	const activateEvent = () => {
		if (effect && fireworks) {
			console.debug("[LifelineEffect] launching from vertical event", {
				effect,
			});
...
export function LifelineVertical({
	...
	const axisLabels = useLifelineAxisLabels();
	...
	return (
		<article
			...
			aria-label={title}
			className={cn(
				"relative select-none px-0 pb-10 pt-0 [&_a]:cursor-pointer",
